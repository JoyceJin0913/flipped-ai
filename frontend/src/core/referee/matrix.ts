 ...
 * 裁判层 Δ 矩阵数据（移植版）
 *
 * 从 src/core/referee/matrix.ts 移植，import 改写：
 *   - ActorIntentType ← ../actorTypes（本地纯类型文件）
 *   - AttachmentType  ← @/onboarding/types
 *
 * 扩展自 scoring.ts 的 BASE_MATRIX，从 5×3 扩展到 9×3（新增 defend/retreat/observe/tease）。
 * 所有人格向量修正权重也集中在此文件，调平入口唯一。
 */

import type { ActorIntentType } from "../actorTypes";
 ...
 */
export function checkCoreNeedHit(intentType: ActorIntentType, attachment: AttachmentType): boolean {
