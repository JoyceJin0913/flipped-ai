...
function avoidantStyle(bannedWords: string[], signatureTokens: string[]): StyleContract {
  return {
    maxCharsPerTurn: 20,
...
export const MALE_NPCS: NPC[] = [
  ...
  makeNpc(
    ...
    ["过度分析", "隐藏自己", "想太多"],
    ["被无条件接纳", "不用伪装"],
    secureStyle(["哈哈", "宝子", "yyds", "绝绝子", "亲"], ["其实", "可能", "你说的", "嗯"], {
      bannedPunctuation: ["!", "~"],
    }),
    secureRules(),
    guyanAvatar,
  ...
  makeNpc(
    ...
    ["敏感多疑", "犹豫不决", "自我怀疑"],
    ["被坚定选择", "作品和人都被欣赏"],
    anxiousStyle(["太差了", "不行", "没救", "丑", "废"], ["那个", "也许", "有点", "不知道"], {
      bannedPunctuation: ["!"],
    }),
    anxiousRules(),
    baizeAvatar,
...
export const FEMALE_NPCS: NPC[] = [
  ...
  makeNpc(
    ...
    ["过分付出", "不懂拒绝", "忽略自己"],
    ["被需要也被关爱", "可以示弱"],
    secureStyle(["滚", "烦死", "关我屁事", "无所谓", "随便"], ["没事", "别担心", "我在", "嗯"], {
      bannedPunctuation: ["!"],
    }),
    secureRules(),
    wenrouAvatar,
