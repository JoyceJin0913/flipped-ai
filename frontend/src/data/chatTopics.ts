 ...
 *
 * @deprecated 新链路请改用 planChatSuggestionSlots + src/lib/chatSuggestions（阶段 1 起）。
 * 按 spec §6.3 保留为确定性规则锚点与旧 UI（HouseApp）的兜底来源，行为与历史版本一致，
 * 因此仍包含「冲突修复话题与冲突记忆跟进同时出现」的历史意图重复问题，待调用点迁移后移除。
 */
...
/** @deprecated getChatTopics 的历史别名，语义与迁移计划相同，见 getChatTopics。 */
export const generateChatTopics = getChatTopics;

/* ------------------------------------------------------------------ *
 * 以下为动态对话选项（阶段 1）的确定性 slot 规划器                    *
 * ------------------------------------------------------------------ */
 ...
 * 冲突情境至多一个 repair——从根上消除旧 getChatTopics 的意图重复问题。
 */
